...
sigue_lineas::~sigue_lineas()
{
}

void sigue_lineas::follow()
{
...
while (ros::ok())
{
    ...
    sigue_lineas::roi(0, rows-100, cols, 100, 0, 0, cols, 130); // Region Of Interest

    sigue_lineas::Threshold();	// Apply treshold

    centro_l = sigue_lineas::find_center_line(centro_l); // Find center of line
    centro_c = sigue_lineas::find_center_curve(centro_c);
...
void sigue_lineas::roi(int x1, int y1, int h1, int w1, int x2, int y2, int h2, int w2)
...
Point sigue_lineas::find_center_line(Point centro_l)
{
...
Point sigue_lineas::find_center_curve(Point centro_c)
{
vector<vector<Point> > contours;
vector<Point> large_contour;
vector<Point> convex_hull;
